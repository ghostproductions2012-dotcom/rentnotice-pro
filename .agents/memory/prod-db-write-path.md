...
- **Deployed build can lag dev self-heals:** verified July 2026 — the live site's overview endpoint returned 404 "License key not found" because the deployed build predated the self-heal that exists in dev code. When verifying prod behavior, remember you are testing the *last published* build, not current dev code; self-heal-dependent fixes only take effect after the next publish.
- Prod schema CAN already be current even without a fresh task-side publish: the Publish flow diffs dev→prod schema each publish, so a rename+new column applied to dev shows up in prod after the user's next publish. Verify with read-only `information_schema.columns` queries before assuming prod is stale.
